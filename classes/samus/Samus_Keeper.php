...
class Samus_Keeper extends Samus_Object {
    ...
    public function displayPage() {
        ...
        if(count($urlDir) > 1) {
             ...
             * primeira maiuscula) e são sempre inseridos e executados quando
             * qualquer classe do pacote são inseridas
             ******************************************************************/
         ...
         *********************************************************************/
        $filterFile = Samus::getControlsDirectory()
                . '/'
                . $directory
                . $filterClass
                . Samus::getControlsFileExtension();
        ...
        if(is_file($filterFile)) {
            ...
            if($ref->getParentClass()->getName() != "Samus_Filter") {
                throw new Exception(
                "A interface Samus_Filter deve ser implementada nos filtros");
            }
        ...
        if(is_file($requireFile)) {
            ...
            if(substr($nClassName, $ajaxTamanho * - 1, $ajaxTamanho) == Samus::getAjaxPhpFileExtension()) {
                ...
                if(substr($className,-8,8)=='.inc.php') {
                    ...
                    $requireViewFile = Samus::getViewsDirectory()
                            . '/'
                            . $directory
                            . UtilString::upperToUnderline($classFile)
                            . Samus::getViewFileExtension();
