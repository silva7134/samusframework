...
/* $Id: Smarty.php 2785 2008-09-18 21:04:12Z Uwe.Tews $ */
...
class Smarty {
	 ...
	 * @var string
	 */
	public $compiler_file = 'Smarty_Compiler.php';
	...
	function fetch($resource_name, $cache_id = null, $compile_id = null, 
		...
		$_smarty_old_error_level = $this->debugging ? error_reporting() : error_reporting(
			isset($this->error_reporting) ? $this->error_reporting : error_reporting() & ~ E_NOTICE);
		...
		if($display && ! $this->caching && count(
			$this->_plugins['outputfilter']) == 0) {
			if($this->_is_compiled($resource_name, $_smarty_compile_path) || $this->_compile_resource(
				$resource_name, 
	...
	function config_load($file, $section = null, $scope = 'global') {
		...
		smarty_function_config_load(
			array('file' => $file , 'section' => $section , 'scope' => $scope), 
			$this);
	}
	...
	function _compile_source($resource_name, &$source_content, 
		...
		if(isset($cache_include_path) && isset(
		...
		$_results = $smarty_compiler->_compile_file(
			$resource_name, 
			$source_content, 
			$compiled_content);
		...
		if($this->_parse_resource_name($_params)) {
			...
			switch ($_resource_type) {
				case 'file':
					...
					$params['resource_timestamp'] = filemtime(
						$_resource_name);
				...
				default:
					...
					if($params['get_source']) {
						$_source_return = isset(
							$this->_plugins['resource'][$_resource_type]) && call_user_func_array(
	...
	function _dequote($string) {
		if((substr($string, 0, 1) == "'" || substr($string, 0, 1) == '"') && substr(
			$string, 
			- 1) == substr($string, 0, 1))
	...
	function _smarty_include($params) {
		...
		if($this->_is_compiled(
			$params['smarty_include_tpl_file'], 
			$_smarty_compile_path) || $this->_compile_resource(
			$params['smarty_include_tpl_file'], 
			$_smarty_compile_path)) {
			include ($_smarty_compile_path);
		}
