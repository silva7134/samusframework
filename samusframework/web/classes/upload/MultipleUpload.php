    ...
    public function save() {
        ...
        foreach($_FILES[$this->inputName]['error'] as $error) {
            if($error > 0) {
                switch ($error) {
                    case 1:
                        throw new UploadException('O arquivo é maior do que o limite definido pelo servidor web');
                        break;
                    case 2:
                        throw new UploadException('O arquivo é maior do que o limite definido pelo sistema<br>');
                        break;
                    case 3:
                        throw new UploadException('O upload do arquivo foi feito parcialmente');
                        break;
                    case 4:
                        throw new UploadException('Não foi feito o upload do arquivo');
                        break;
                }
            }
        }
        ...
        foreach($_FILES[$this->inputName]['name'] as $key => $name) {
            ...
            if(! move_uploaded_file($_FILES[$this->inputName]['tmp_name'][$key], $this->getDir() . $nameOfFile)) {
                throw new UploadException("ERRO ! Possivel ataque de upload! Verifique se o diretorio especificado existe e se tem permissão de escrita");
            }
